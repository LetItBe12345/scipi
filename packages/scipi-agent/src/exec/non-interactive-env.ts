...
const POSIX_ENV_PASSTHROUGH_KEYS = [
	"HOME",
	"PATH",
	"USER",
	"LOGNAME",
...
export function buildNonInteractiveEnv(
	...
	if (platform !== "win32") {
		const inherited = pickPassthroughEnv(baseEnv, POSIX_ENV_PASSTHROUGH_KEYS);
		return overrides ? { ...inherited, ...NON_INTERACTIVE_ENV, ...overrides } : { ...inherited, ...NON_INTERACTIVE_ENV };
	}
