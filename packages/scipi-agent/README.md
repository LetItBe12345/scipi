# @scipi/scipi-coding-agent

Core implementation package for the `scipi` coding agent in the `oh-my-pi` monorepo.
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.scipi/config.yml`):
