...
describe("createAgentSession MCP discovery prompt gating", () => {
	...
	it("default discovery keeps MCP tools behind search when the tool set is too large", async () => {
	...
	it("keeps enabled long-tail built-ins discoverable but inactive under tools.discoveryMode all", async () => {
		const { session } = await createAgentSession({
			cwd: tempDir,
			agentDir: tempDir,
			modelRegistry,
		...
		const activeNames = session.getActiveToolNames();
		const discoverableNames = session.getDiscoverableTools({ source: "builtin" }).map(tool => tool.name);
		expect(activeNames).not.toContain("browser");
		expect(activeNames).not.toContain("debug");
		expect(activeNames).not.toContain("systemd");
		expect(discoverableNames).toEqual(expect.arrayContaining(["browser", "debug", "systemd"]));
	...
	it("keeps task active under tools.discoveryMode all when task.eager is default", async () => {
		const { session } = await createAgentSession({
			cwd: tempDir,
			agentDir: tempDir,
			modelRegistry,
	...
	it("keeps web_search active under tools.discoveryMode all", async () => {
		const { session } = await createAgentSession({
			cwd: tempDir,
			agentDir: tempDir,
			modelRegistry,
